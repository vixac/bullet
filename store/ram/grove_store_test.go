...
func TestGroveDescendants(t *testing.T) {
	...
	a := store_interface.NodeID("a")
	b := store_interface.NodeID("b")
	c := store_interface.NodeID("c")
	d := store_interface.NodeID("d")
...
func TestGroveSoftDelete(t *testing.T) {
	...
	root := store_interface.NodeID("root")
	child := store_interface.NodeID("child")
	...
	// Restore node
	err = store.RestoreNode(space, child)
	if err != nil {
	...
	exists, _ = store.Exists(space, child)
	if !exists {
		t.Error("Restored node should exist")
	}
}

func TestGroveMultiTenancy(t *testing.T) {
	store := NewRamStore()
	space1 := store_interface.TenancySpace{AppId: 1, TenancyId: 1}
	space2 := store_interface.TenancySpace{AppId: 1, TenancyId: 2}

	nodeID := store_interface.NodeID("node1")
