...
func TestWarehousePostgresPersistenceAndBatch(t *testing.T) {
 ...
 INSERT INTO depot (app_id, tenancy_id, bucket_id, value) VALUES (1,2,3,'existing')`)
	...
	ctx := context.Background()
	space := si.TenancySpace{AppId: 1, TenancyId: 2}
	req := si.PutBlobRequest{PutID: "persist", Value: []byte{0, 255, 0}, Checksum: "opaque"}
	original, err := s.WarehousePut(ctx, space, req)
	require.NoError(t, err)
	...
	ids := make([]si.BlobID, 2100)
	for i := range ids {
		ids[i] = si.BlobID(fmt.Sprint(i))
	}
	expected := map[si.BlobID]si.Blob{original.ID: original}
	ids[0] = original.ID
	for _, i := range []int{999, 1000, 1999, 2000, 2099} {
		b, err := s.WarehousePut(ctx, space, si.PutBlobRequest{PutID: si.PutID(fmt.Sprint(i)), Value: []byte{255, 0}})
		require.NoError(t, err)
		ids[i] = b.ID
		expected[b.ID] = b
	}
...
func TestWarehousePostgresConcurrentInstances(t *testing.T) {
	...
	for i := 0; i < 32; i++ {
		go func(i int) {
			...
			b, err := stores[i%2].WarehousePut(context.Background(), si.TenancySpace{}, si.PutBlobRequest{PutID: "race", Value: []byte(value)})
...
func TestWarehousePostgresHTTP(t *testing.T) {
	...
	req := si.PutBlobRequest{PutID: "http", Value: []byte{255, 0}}
	...
	many := request("POST", "/warehouse/blobs/batch-get", gin.H{"ids": []si.BlobID{b.ID, "missing"}})
	require.Equal(t, 200, many.Code)
	var found map[si.BlobID]si.Blob
	require.NoError(t, json.Unmarshal(many.Body.Bytes(), &found))
	require.Equal(t, map[si.BlobID]si.Blob{b.ID: b}, found)
	req.Checksum = "changed"
	...
	require.Equal(t, 400, request("POST", "/warehouse/blobs", si.PutBlobRequest{}).Code)
}
